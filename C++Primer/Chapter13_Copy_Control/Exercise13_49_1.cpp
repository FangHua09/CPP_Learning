/*
练习 13.49：为 StrVec、String、Message 添加移动构造、移动赋值
知识点：移动构造 / 移动赋值，参数是类名 &&，接管源对象底层资源，源对象置空，标记noexcept（STL 容器扩容时会依赖 noexcept 判断是否使用移动）
核心逻辑：直接窃取指针，不分配内存、不拷贝元素；源对象成员置为 nullptr，防止析构重复释放。
*/
#include <iostream>
#include <vector>
#include <string>
...
void StrVec::reallocate()
{
    ...
    // 使用移动迭代器优化
    auto last = uninitialized_copy(move_iterator(begin()), move_iterator(end()), newp);
    free();
    elements = newp;
    first_free = last;
...
// 移动构造
StrVec::StrVec(StrVec &&orig)noexcept:elements(orig.elements), cap(orig.cap), first_free(orig.first_free)
{
    //把orig移动到*this
    cout << "StrVec【移动构造】" << endl;
    orig.elements = orig.first_free = orig.cap = nullptr; // 置于可析构状态
}
...
StrVec &StrVec::operator=(StrVec &&rhs)noexcept{
    ...
    cap = rhs.cap;
    rhs.elements = rhs.first_free = rhs.cap = nullptr; // 右侧运算对象置于可析构状态
    return *this;
}
