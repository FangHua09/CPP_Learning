...
知识点
在 QueryResult 添加成员函数begin()、end()，返回 set 的迭代器；
添加成员函数get_file()返回保存文本的 shared_ptr<vector<string>>。
解答
修改 QueryResult 类，增加三个成员函数：
*/
...
public:
    using line_no = vector<string>::size_type;
    using set_iter = set<line_no>::iterator;
    using set_const_iter = set<line_no>::const_iterator;
    ...
    // 添加begin和end成员， 返回一个迭代器， 指向一个给定查询返回行号中的位置
    // 定制const版本，因为避免拷贝的参数将会调用常函数版本
    set_iter begin() { return lines->begin(); }
    set_iter end() { return lines->end(); }
    // 常量版本
    set_const_iter cbegin() const { return lines->cbegin(); }
    set_const_iter cend() const { return lines->cend(); }
    set_const_iter begin() const { return lines->end(); }
    set_const_iter end() const { return lines->end(); }

    // 返回get_file()
    shared_ptr<vector<string>> get_file(){return file.get_data();}
